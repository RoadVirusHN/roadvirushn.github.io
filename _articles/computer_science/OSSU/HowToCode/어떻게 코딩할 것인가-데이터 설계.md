...
강의에서는 재귀를 이용해 리스트를 처리하지만, 대부분의 프로그래밍 언어에서는 그러하지 않으므로 변경했다.

대신, 재귀 함수에 대한 팁을 적어놓았다.
...
- 기저 사례를 맨 처음 찾아내고, 처리하도록 테스트 생성 및 구현
	- 구현이 간단한 경우가 많고, 재귀 함수 구현을 위해 꼭 필요하므로 최초로 디버깅되게 하기 위함
- 재귀 사례는 결국 기저 사례로 향하는지 알기 위해, 넘겨주는 인자가 기저 사례로 수렴하도록 변경되는지 확인하기

```

### 여러 종류의 데이터 정의가 필요한 경우
