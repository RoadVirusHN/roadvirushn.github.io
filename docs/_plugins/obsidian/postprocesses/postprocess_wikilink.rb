module PostprocessWikilink
  ...
  def convert_wikilink(str)
    str.gsub(EXTERNAL_LINK_REGEX) do |matched|
      change_external_link(Regexp.last_match(1), matched)
    end
    .gsub(INTERNAL_LINK_REGEX) do |matched|
  ...
  def change_external_link(href, a_tag)
    "<span class='link-warning'>
      #{a_tag}
      <span class='link-warning-text'>보안 확인 : <em>외부링크</em><br/><em>\"#{href.length > 30 ? "#{href.slice(0, 30)}..." : href}\"</em>로 이동?<br/>
