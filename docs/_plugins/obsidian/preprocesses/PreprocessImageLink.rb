module PreprocessImageLink 
    OBSIDIAN_IMAGE_LINK_REGEX = /!\[\[(?:(?!https?:\/\/)(?:.*\/)?([^\[\]\|]+\.[a-z]{3,4}))??(https?:\/\/(?:www\.)?[-a-zA-Z0-9@:%._\\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9(@:%_\+.~#?&\/=]*))?(?:\|(.*))?\]\](?=[^`]*(?:`[^`]*`[^`]*)*\Z)/
    MARKDOWN_IMAGE_LINK_REGEX =  /!\[(?:([^\[\]\|]+))?\]\((?:(?!https?:\/\/)(?:.*\/)?([^\[\]\|]+\.[a-z]{3,4}))?(https?:\/\/(?:www\.)?[-a-zA-Z0-9@:%._\\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b(?:[-a-zA-Z0-9(@:%_\+.~#?&\/=]*))?\)(?=[^`]*(?:`[^`]*`[^`]*)*\Z)/
